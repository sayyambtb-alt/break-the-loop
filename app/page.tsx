...
export default function Home() {
  ...
  const registerAndMatch = async (lat: number, lng: number, currentRoom: string) => {
    ...
    try {
      // 1. Look for ANY other device in queue created in the last 3 minutes for the same mode
      if (mode !== 'solo' && !isInviteSession) {
        ...
        if (activeOthers && activeOthers.length > 0) {
          matchFound = true;
          finalRoomId = activeOthers[0].status; // Hijack partner's room ID
          finalQuest = activeOthers[0].active_quest; // Sync to partner's quest
          setRoomId(finalRoomId);
          setMatchedPartner(`Matched: Live Local Partner nearby! 🤝`);
        }
      }

      // 2. If no partner was in the queue, select a brand new quest from DB
      if (!finalQuest) {
      ...
      // 3. Upsert THIS device's entry into active_queue so other devices find it!
...
EOFgit add app/page.tsx
git commit -m "Fix queue query ordering and instant match hijacking"
git push
