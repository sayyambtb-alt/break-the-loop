...
describe('duo/squad shared reroll', () => {
  it('calls reroll_shared_quest and syncs the new quest via the realtime queue subscription', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /roll a mission/i }));
    ...
    expect(screen.getByText('+75 XP')).toBeInTheDocument();
             ...
             // Vitest's default 5s test timeout under any CI load, even
             // when nothing is actually broken -- this was the real cause
             // of the recurring flaky re-runs, not an app bug.
});
